...
export function createXlsx(sheets: Sheet[]): Uint8Array {
    ...
    sheets.forEach((sheet, i) => {
        ...
        add(`xl/worksheets/sheet${i+1}.xml`, `<worksheet xmlns="${ns}"><sheetViews><sheetView workbookViewId="0"><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><cols><col min="1" max="${width}" width="26" customWidth="1"/></cols><sheetData>${sheet.rows.map((row, r) => `<row r="${r+1}">${row.map((cell, c) => {
