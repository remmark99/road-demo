...
export function createXlsx(sheets: Sheet[]): Uint8Array {
    ...
    add('xl/styles.xml', `<styleSheet xmlns="${ns}"><fonts count="2"><font><sz val="11"/><name val="Calibri"/></font><font><b/><color rgb="FF303030"/><sz val="11"/><name val="Calibri"/></font></fonts><fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FFFFFFFF"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="2"><border/><border><left style="thin"><color rgb="FF808080"/></left><right style="thin"><color rgb="FF808080"/></right><top style="thin"><color rgb="FF808080"/></top><bottom style="thin"><color rgb="FF808080"/></bottom><diagonal/></border></borders><cellStyleXfs count="1"><xf/></cellStyleXfs><cellXfs count="4"><xf xfId="0" fontId="0" fillId="0" borderId="0"/><xf xfId="0" fontId="1" borderId="1" applyBorder="1" fillId="2" applyFill="1" applyFont="1" applyAlignment="1"><alignment horizontal="center" vertical="center" wrapText="1"/></xf><xf xfId="0" borderId="1" applyBorder="1" fillId="2" applyFill="1" applyAlignment="1"><alignment wrapText="1" horizontal="center" vertical="center"/></xf><xf xfId="0" borderId="1" applyBorder="1" fillId="2" applyFill="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>`)
    sheets.forEach((sheet, i) => {
        ...
        add(`xl/worksheets/sheet${i+1}.xml`, `<worksheet xmlns="${ns}" xmlns:r="${rel}"><sheetViews><sheetView workbookViewId="0" showGridLines="0"><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><cols>${Array.from({ length: width }, (_, c) => `<col min="${c+1}" max="${c+1}" width="${sheet.columnWidths?.[c] ?? 26}" customWidth="1"/>`).join('')}</cols><sheetData>${sheet.rows.map((row, r) => `<row r="${r+1}"${sheet.wrapColumns?.length && r > 0 ? ` ht="${Math.max(1, ...sheet.wrapColumns.map(c => String(row[c] ?? '').split('\n').reduce((n, line) => n + Math.max(1, Math.ceil(line.length / (sheet.columnWidths?.[c] ?? 26))), 0))) * 16}" customHeight="1"` : ''}>${row.map((cell, c) => {
            const ref = `${column(c)}${r+1}`, style = r === 0 ? ' s="1"' : sheet.wrapColumns?.includes(c) ? ' s="2"' : ' s="3"'
