...
export function cameraEventSheet(alerts:Alert[], episodes:Map<string,LyingPersonEpisode>, place:(a:Alert)=>string, label:(type:string)=>string, now=Date.now()):Sheet {
  return {name:'События камер',columnWidths:[38,30,22,22,23,20],wrapColumns:[0,1],rows:[['Место','Событие','Начало','Завершение','Длительность','Статус'],...alerts.map(a=>{
    const episode=getBinEpisode(a)||getAbandonedEpisode(a)||episodes.get(String(a.metadata?.episode_id))
    return [place(a),label(a.alert_type),reportTime(episode?.started_at||a.timestamp),episode?.status==='open'?'Продолжается':reportTime(episode?.ended_at),episode?formatEventDuration(episode.started_at,episode.status==='open'?new Date(now).toISOString():episode.ended_at||episode.last_seen_at):'—',episode?(episode.status==='open'?'Продолжается':'Завершено'):'Зафиксировано']
  })]}
}
export function sensorEventSheet(alerts:ControllerAlert[],place:(a:ControllerAlert)=>string,label:(a:ControllerAlert)=>string,status:(alarm:string)=>string):Sheet {
 return {name:'События датчиков',columnWidths:[38,38,22,22,23,20],wrapColumns:[0,1],rows:[['Место','Событие','Начало','Завершение','Длительность','Статус'],...alerts.map(a=>{
  ...
  return [place(a),label(a),reportTime(start),recovered?reportTime(a.created_at):'—',recovered?formatEventDuration(start,a.created_at):'—',recovered?'Связь восстановлена':status(a.alarm)]
 })]}
}
