...
const serviceConfig = {
    doors_not_opened: { label: "Неоткрытые двери", color: "hsl(347, 89%, 62%)" },
...
const TYPE_ICONS: Record<TransportIncidentType, typeof Route> = {
    route_deviation: Route,
    wait_overrun: Clock,
    doors_not_opened: DoorClosed,
}
...
export function TransportServiceAnalytics() {
    ...
    const filteredIncidents = useMemo(() => {
        return filterByTimeRange(
            ...
            timeRange
        ).filter((incident) => incident.type !== "route_deviation")
    }, [selectedRoutes, timeRange])

    const dailyData = useMemo(() => {
        const map = new Map<
            string,
            { dateLabel: string; doors_not_opened: number; wait_overrun: number }
        >()
        ...
        for (const item of filteredDaily) {
            ...
            row.doors_not_opened += item.doors_not_opened
        ...
        return TRANSPORT_ROUTES.filter((route) => selectedRoutes.includes(route.id)).map((route) => {
            ...
            const avgWait =
                rows.length === 0
                    ? 0
                    : Math.round(
                        rows.reduce((sum, item) => sum + item.avgWaitMinutes, 0) / rows.length
                    )

            return {
                route: route.name,
                doors_not_opened: rows.reduce((sum, item) => sum + item.doors_not_opened, 0),
    ...
    const totals = useMemo(() => {
        const result = filteredDaily.reduce(
            (acc, item) => {
                acc.doors_not_opened += item.doors_not_opened
    ...
    return (
        <div className="h-full overflow-auto p-6 space-y-6">
                    ...
                    <PopoverContent className="w-80" align="start">
                        <div className="space-y-3">
                            <div className="flex items-center space-x-2">
                                <Checkbox
                                    id="all-routes-service"
                                    checked={selectedRoutes.length === TRANSPORT_ROUTES.length}
                                    onCheckedChange={toggleAll}
                                />
                                <Label htmlFor="all-routes-service" className="font-medium">
                                    Все маршруты
                                </Label>
                            </div>
                            <div className="border-t pt-2 space-y-2">
                                {TRANSPORT_ROUTES.map((route) => (
                                    <div key={route.id} className="flex items-center space-x-2">
                                        <Checkbox
                                            id={`transport-service-${route.id}`}
                                            checked={selectedRoutes.includes(route.id)}
                                            onCheckedChange={() => toggleRoute(route.id)}
                                        />
                                        <Label htmlFor={`transport-service-${route.id}`} className="text-sm">
                                            {route.name}
                                        </Label>
                                    </div>
                                ))}
                ...
                <Card className="bg-gradient-to-br from-rose-500/10 to-rose-600/5 border-rose-500/20">
                    <CardContent className="pt-4 pb-3 px-4">
                        <div className="flex items-center gap-2 mb-1">
                            <DoorClosed className="h-4 w-4 text-rose-500" />
                            <span className="text-xs text-muted-foreground">Неоткрытые двери</span>
                        </div>
                        <div className="text-2xl font-bold">{totals.doors_not_opened}</div>
                    </CardContent>
                </Card>
                ...
                <Card className="bg-gradient-to-br from-sky-500/10 to-sky-600/5 border-sky-500/20">
                    <CardContent className="pt-4 pb-3 px-4">
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-xs text-muted-foreground">Ср. задержка</span>
                        </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                <Card className="xl:col-span-2">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-base flex items-center gap-2">
                            ...
                            Качество обслуживания по дням
                        </CardTitle>
                        <CardDescription>
                            Инциденты обслуживания пассажиров на остановках
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <ChartContainer config={serviceConfig} className="h-[300px] w-full">
                            <BarChart data={dailyData} margin={{ left: 0, right: 12, top: 10, bottom: 0 }}>
                                <CartesianGrid vertical={false} strokeDasharray="3 3" />
                                <XAxis dataKey="dateLabel" tickLine={false} axisLine={false} />
                                <YAxis tickLine={false} axisLine={false} allowDecimals={false} />
                                <ChartTooltip content={<ChartTooltipContent />} />
                                <ChartLegend content={<ChartLegendContent />} />
                                <Bar dataKey="doors_not_opened" fill="var(--color-doors_not_opened)" radius={[4, 4, 0, 0]} />
                ...
                <Card>
                    <CardHeader className="pb-2">
                        ...
                        <CardDescription>
                            Где чаще возникают сервисные сбои
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <ChartContainer config={routeConfig} className="h-[300px] w-full">
            ...
            <Card>
                <CardHeader className="pb-2">
                    <CardTitle className="text-base">Журнал обслуживания</CardTitle>
                    <CardDescription>
                        Последние события по дверям и ожиданию на остановках
                    </CardDescription>
                </CardHeader>
                <CardContent>
                                        ...
                                        <div className="space-y-1">
                                            <div className="flex flex-wrap items-center gap-2">
                                                ...
                                                <Badge variant={incident.resolved ? "secondary" : "outline"}>
                                                    {incident.resolved ? "Закрыт" : "Ожидает"}
                                                </Badge>
                                            </div>
