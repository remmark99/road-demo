...
export function EventsExport({channel,period,filters}:{channel:'cameras'|'sensors';period:NotificationPeriod;filters:Record<string,string|undefined>}) {
  ...
  return <div className="mb-3 flex flex-wrap items-center justify-end gap-2"><Button variant="outline" size="sm" disabled={busy} onClick={download}>{busy ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <Download className="mr-2 h-4 w-4"/>}{busy ? 'Формирование…' : 'Скачать события в Excel'}</Button>{error && <p role="alert" className="text-sm text-destructive">{error}</p>}</div>
}
